...
bool nextImage(VideoCapture & inputCapture, Mat & result, long * millis_timestamp)
{
	...
	cout<<"CV_CAP_PROP_POS_MSEC: " << convertToEpoch(videoCap.get(CV_CAP_PROP_POS_MEC)) <<"\n" <<
		  "CV_CAP_PROP_POS_FRAMES: "<< (long)videoCap.get(CV_CAP_PROP_POS_FRAMES) << "\n" <<  // <-- the v4l2 'sequence' field
		  "CV_CAP_PROP_FPS:  "<< videoCap.get(CV_CAP_PROP_FPS) << "\n";

	if (millis_timestamp)*millis_timestamp = convertToEpoch(videoCap.get(CV_CAP_PROP_POS_MEC));

	return true;
}

int init_chessboard_navigation(int camera_id, volatile bool * stop_flag )
{
	...
	for (int re_connect_retries = 0;!(*stop_flag);++connect_retries)
	{
